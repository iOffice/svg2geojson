...
function convertSVGToGeoJSON(doc, options={}) {
	...
	if (!corners || corners.length != 2) {
        corners = options.corners.map(corner => {
            // Uppercase all keys to conform to the parsed xml format
            const result = Object.keys(corner).reduce((obj, key) => {
                const uppercaseKey = key.charAt(0).toUpperCase() + key.substring(1, key.length);
                obj[uppercaseKey] = { value: corner[key] };
                return obj;
            }, {});
            return { $: result };
        });
